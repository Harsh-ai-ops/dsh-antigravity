...
// Thought-signature cache: thinking models attach a thoughtSignature to every
// function call, and the Gemini API REQUIRES it to be replayed with the
// functionCall in later turns ("Function call is missing a thought_signature"
// 400 otherwise). The OpenAI wire format has no field for it, so we cache
// signature by the tool_call id we mint when emitting the call, and re-attach
// it when the client replays that id in history. Cache lives for the process
// lifetime; ids are UUIDs so collisions across restarts are impossible.
const thoughtSignaturesByCallId = new Map();
const THOUGHT_SIG_MAX = 5000; // bound memory; drop oldest
function rememberThoughtSignature(callId, signature) {
  if (!callId || typeof signature !== "string" || !signature) return;
  if (thoughtSignaturesByCallId.size >= THOUGHT_SIG_MAX) {
    const oldest = thoughtSignaturesByCallId.keys().next().value;
    thoughtSignaturesByCallId.delete(oldest);
  }
  thoughtSignaturesByCallId.set(callId, signature);
}

export function openaiToGemini(chatBody) {
  const contents = [];
  const toolCallNameById = new Map();
  const toolCallSignatureById = new Map();

  // Signatures may ride on the same part as the function call or on a
  // preceding sibling part; remember both shapes, matched in emission order.
    ...
    if (m.role === "assistant") {
      ...
      if (Array.isArray(m.tool_calls)) {
        for (const tc of m.tool_calls) {
          ...
          // re-attach the cached thought signature as a SIBLING of
          // functionCall (upstream shape: {thoughtSignature, functionCall});
          // nesting it inside functionCall is a 400 'Cannot find field'
          const sig = thoughtSignaturesByCallId.get(tc.id);
...
export async function geminiStreamToOpenAI(geminiRes, res, model) {
  ...
  let toolIdx = 0;
  let pendingStreamSignature = null; // standalone sig part awaiting its functionCall
  let buffer = "";
  let emittedAnything = false; // text or tool_calls emitted?
  let sawThoughtsOnly = false; // parts had thought signatures but no text
  let streamError = null; // error event mid-stream
  ...
  try {
    while (true) {
      ...
      if (done) break;
      dbgBytes += value.byteLength;
      if (process.env.AGW_STREAM_DEBUG) {
        console.log(`[agw-dbg] read chunk ${value.byteLength}B total=${dbgBytes} head=${decoder.decode(value.slice(0, 80))}`);
      }
      buffer += decoder.decode(value, { stream: true });

      // Upstream SSE may use LF or CRLF line endings. Normalize ALL CRLF to
      // LF first: in a "\r\n\r\n"-separated stream, indexOf("\n\n") never
      // matches ("\r\n\r\n" does not contain two consecutive LFs), so
      // without normalization NOTHING is ever parsed and every stream
      // comes back empty.
      ...
      while ((idx = buffer.indexOf("\n\n")) >= 0) {
        ...
        for (const line of rawEvent.split("\n")) {
          ...
          // Error events (quota exhausted mid-stream etc.) arrive as
          // {error: {...}} with no candidates. Capture instead of skipping.
          if (ev.error) {
          ...
          for (const part of cand.content?.parts || []) {
            if (typeof part.text === "string" && part.text) {
              ...
              // standalone signature part preceding its function call
              sawThoughtsOnly = true;
              pendingStreamSignature = part.thoughtSignature;
            }
            if (part.functionCall) {
              emittedAnything = true;
              const callId = "call-" + crypto.randomUUID();
              // capture the signature riding WITH the call (or the pending
              // standalone one) so history replay can re-attach it
              const sig = part.functionCall.thoughtSignature || part.thoughtSignature || pendingStreamSignature;
              pendingStreamSignature = null;
              if (sig) rememberThoughtSignature(callId, sig);
              sseWrite(res, {
                id,
                object: "chat.completion.chunk",
                created: Math.floor(Date.now() / 1000),
    ...
    if (process.env.AGW_STREAM_DEBUG) {
      console.log(`[agw-dbg] stream done: bytes=${dbgBytes} emitted=${emittedAnything} thoughtsOnly=${sawThoughtsOnly} err=${streamError ? streamError.message : "none"}`);
    }
    // Never end a 200 stream silently empty: thinking models can burn the
    // whole budget on hidden reasoning (thought-signature parts, no text) or
    // quota can die mid-stream — a bare finish_reason:"stop" would read as
    // "completed with no content" downstream. Emit a visible explanation
    // instead so clients (and their users) see a real message.
    if (!emittedAnything) {
      const reason = streamError
        ? `upstream error: ${streamError.message || JSON.stringify(streamError).slice(0, 300)}`
        : sawThoughtsOnly
          ? "model produced only hidden reasoning (thought budget exhausted before any output token); try again, simplify the request, or switch to a non-thinking model"
          : "model returned an empty response (no content parts in any upstream event)";
      sseWrite(res, {
        id,
        object: "chat.completion.chunk",
        created: Math.floor(Date.now() / 1000),
        model,
        choices: [{ index: 0, delta: { content: `[antigravity] ${reason}` }, finish_reason: null }],
      });
      if (streamError) {
        console.warn(`[dsh-antigravity] Stream ended with upstream error: ${streamError.message}`);
      }
    } else if (streamError) {
      // partial content was emitted, then the stream errored — append the
      // error note so the truncation is visible instead of silent
      sseWrite(res, {
        id,
        object: "chat.completion.chunk",
        created: Math.floor(Date.now() / 1000),
        model,
        choices: [{ index: 0, delta: { content: `\n[antigravity] stream ended early: ${streamError.message || "upstream error"}` }, finish_reason: null }],
      });
    }
    sseWrite(res, {
      id,
...
function aggregateUpstream(text) {
  let outText = "";
  const toolCalls = [];
  // signature may ride as a sibling of functionCall or as a standalone part
  let pendingSig = null;
  const consumeChunk = (chunk) => {
    const cand = chunk.response?.candidates?.[0] ?? chunk.candidates?.[0];
    for (const part of cand?.content?.parts ?? []) {
      if (typeof part.text === "string") outText += part.text;
      if (part.thoughtSignature && !part.functionCall) pendingSig = part.thoughtSignature;
      if (part.functionCall) {
        const fc = { ...part.functionCall };
        if (part.thoughtSignature) fc.thoughtSignature = part.thoughtSignature;
        else if (pendingSig) fc.thoughtSignature = pendingSig;
        pendingSig = null;
        toolCalls.push(fc);
      }
    }
  ...
  if (trimmed.startsWith("[")) {
    ...
    // normalize CRLF (upstream SSE uses \r\n line endings)
...
export function startGateway(port = 8787) {
  ...
  async function handleChat(req, res) {
    ...
    while (attempts-- > 0) {
      ...
      // A thinking model can burn its whole budget on hidden reasoning —
      // every part is a thought signature, outText ends up empty. Returning
      // content:null makes clients fail with "completed with no content";
      // return a visible explanation instead.
      let finalText = outText;
      if (!finalText && !toolCalls.length) {
        const thoughtsOnly = /thoughtSignature|"thought"/.test(text);
        const errMatch = text.match(/"message":\s*"([^"]+)"/);
        finalText = errMatch
          ? `[antigravity] upstream error: ${errMatch[1]}`
          : thoughtsOnly
            ? "[antigravity] model produced only hidden reasoning (thought budget exhausted before any output token); try again, simplify the request, or switch to a non-thinking model"
            : "[antigravity] model returned an empty response (no content parts in any upstream event)";
      }
      const message = { role: "assistant", content: finalText };
      if (toolCalls.length) {
        // capture each call's thought signature under the id we mint so the
        // replay path can re-attach it (Gemini requires signatures on
        // functionCall replay for thinking models)
        message.tool_calls = toolCalls.map((fc) => {
          const callId = "call-" + crypto.randomUUID();
          if (fc.thoughtSignature) rememberThoughtSignature(callId, fc.thoughtSignature);
          return {
            id: callId,
            type: "function",
