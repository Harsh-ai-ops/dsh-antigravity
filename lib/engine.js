  ...
  async function handleChat(req, res) {
    ...
    while (attempts-- > 0) {
      ...
      if (upstream.status === 403) {
        // 403 PERMISSION_DENIED / SUBSCRIPTION_REQUIRED (e.g. account has no
        // Antigravity license) is a per-account condition, not a request
        // problem: quarantine this account (1h) and fail over instead of
        // killing the request. If it's a real request error (invalid body),
        // every account will reject it and the loop exhausts with the text.
        const errText = await upstream.text();
        const licenseIssue = /SUBSCRIPTION_REQUIRED|valid license/i.test(errText);
        markRateLimit(store, storePath, key.id || key.name, licenseIssue ? "license" : model, licenseIssue ? 3600000 : 60000);
        if (licenseIssue) {
          console.warn(`[dsh-antigravity] Account ${key.email || key.id || key.name} has no Antigravity license (403) — quarantined for 1h, failing over`);
          continue;
        }
