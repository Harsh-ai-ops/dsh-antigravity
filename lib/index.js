 ...
 * Keeps the callback server alive across multiple logins until the user clicks 'Done'.
 */
function autoTriggerFirstTimeLogin() {
  ...
  stopServer = startOAuthCallbackServer(async (code, state) => {
    const keys = loadKeys();
    const name = `account-${keys.length + 1}`;
    try {
      const res = await exchangeCodeForTokens(code, state);
      addKey(name, res.refresh, res.email, res.projectId);
      console.log(`\n[dsh-antigravity] Linked account ${keys.length + 1}: ${res.email}`);
    } catch (err) {
      console.error(`[dsh-antigravity] Login error: ${err.message}`);
    }
  }, 51121);

  // Auto open the initial consent screen in user's default browser
  const auth = generateAuthUrl("");
  openUrlInBrowser(auth.url);

  // Leave server running for 10 minutes so user can link as many accounts as they want
  setTimeout(() => {
    if (stopServer) {
...
export function apply(ctx, config) {
  ...
  ctx.inject(["tools"], (hostCtx) => {
    ...
    tools.register({
      ...
      description: "Check currently connected Google Antigravity accounts, which active account is in use, and quota percentage remaining/used per model.",
      ...
      async execute() {
        ...
        if (keys.length === 0) {
          return {
            ...
            message: "No Antigravity accounts connected yet. Run antigravity_login to connect.",
            accounts: [],
          };
        }
        ...
        for (const k of keys) {
          let quotaInfo = null;
          try {
            const auth = await refreshAccessToken(k.key);
            if (auth?.accessToken) {
              const buckets = await fetchUserQuota(auth.accessToken);
              if (buckets && Array.isArray(buckets)) {
                // Group by main model families
                const modelUsage = {};
                for (const b of buckets) {
                  if (!b.modelId || b.modelId.startsWith("chat_") || b.modelId.startsWith("tab_")) continue;
                  const remainingPct = Math.round((b.remainingFraction ?? 1) * 100);
                  const usedPct = 100 - remainingPct;
                  modelUsage[b.modelId] = {
                    remaining_percentage: `${remainingPct}%`,
                    used_percentage: `${usedPct}%`,
                    resets_at: b.resetTime || "weekly",
                  };
                }
                quotaInfo = modelUsage;
              }
            }
          } catch (e) {
            quotaInfo = { error: e.message };
          }
        ...
        return {
          ...
          active_account: keys[0]?.email || keys[0]?.name,
