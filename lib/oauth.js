...
export function openUrlInBrowser(url) {
  const platforms = [
    // Windows: `cmd /c start` re-parses its whole command line, and an unquoted
    // URL gets truncated at the first `&` (OAuth URLs are full of them), so
    // Google sees only client_id and returns "missing response_type".
    // Quoting the URL keeps ampersands inside one cmd token.
    { test: /^win/, cmd: "cmd", args: ["/c", "start", "", `"${url}"`] },
    { test: /^darwin/, cmd: "open", args: [url] },
    { test: /.*/, cmd: "xdg-open", args: [url] },
  ];
  const platformKey = process.platform;
  const choice = platforms.find((p) => p.test.test(platformKey)) || platforms[2];
  try {
    spawn(choice.cmd, choice.args, { detached: true, stdio: "ignore" }).unref();
    return true;
  } catch {
    return false;
  }
